from dash import Dash, dcc, html, dash_table, Input, Output, no_update, callback, State, clientside_callback
...
for typ, col in typ_col_pairs:
    ...
    type_dict[col] = typ
#####################################################################################
def make_dash(server, external_stylesheets):
    external_stylesheets = [dbc.themes.BOOTSTRAP, "../static/css/image_annotation_style.css"]
    return dash.Dash(server=server, url_base_pathname='/training/', external_stylesheets=external_stylesheets)
...
def make_layout(config):
    ...
    with open("static/assets/Howto.md", "r") as f:
        # Using .read rather than .readlines because dcc.Markdown
        # joins list of strings with newline characters
        howto = f.read()

    filelist = os.listdir("./static/record/NYCU/img_detect/2023-03-27/08")
    full_path = []
    for file in filelist:
        path = os.path.join("./static/record/NYCU/img_detect/2023-03-27/08", file)
        full_path.append(path)
    full_path.sort()

    fig = px.imshow(io.imread(full_path[0]), binary_backend="jpg")
    fig.update_layout(
                    ...
                    dbc.Row(
                        dbc.Col(
                            [
                                ...
                                dcc.Store(
                                    id="image_files",
                                    data={"files": full_path, "current": 0},
                                ),
                            ],
                        ),
                    ),
                    ...
                    html.Div(
                        [
                            ...
                            dbc.Button(
                                ...
                                style={"background-color": "rgba(108, 117, 125, 0)"},
                            ),
    ...
    navbar = dbc.Navbar(
        dbc.Container(
            ...
            fluid=True,
            style={"background-color": "#2070c0!important"}
        ),
        color="dark",
        dark=True,
        className="mb-5",
